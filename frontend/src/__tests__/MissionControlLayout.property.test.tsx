...
import { WorkflowState, AgentType, AgentStatus, RiskLevel, HealthStatus, AlertSeverity } from '../types/workflow';
...
jest.mock('lucide-react', () => {
  ...
  return new Proxy({}, { get: (_, prop) => typeof prop === 'string' ? createMockIcon(prop) : undefined });
});

jest.mock('framer-motion', () => ({
  motion: {
    div: ({ children, initial, animate, exit, variants, transition, whileHover, whileTap, ...rest }: any) => 
      React.createElement('div', rest, children),
    span: ({ children, initial, animate, exit, variants, transition, whileHover, whileTap, ...rest }: any) => 
...
const renderWithProvider = () => {
  cleanup();
  return render(React.createElement(WorkflowProvider, null, React.createElement(MissionControl)));
};

const workflowStateArb = fc.constantFrom(
  WorkflowState.IDLE, WorkflowState.INCIDENT_INGESTED, WorkflowState.ANALYZING,
  WorkflowState.RCA_COMPLETE, WorkflowState.GOVERNANCE_PENDING, WorkflowState.ACTION_PROPOSED,
...
describe('Property 15: Mission Control Layout Consistency', () => {
  ...
  describe('5.1 Three-Pane Layout Structure', () => {
    test('should always render exactly three panels regardless of workflow state', () => {
      fc.assert(fc.property(workflowStateArb, () => {
        ...
        expect(container.querySelectorAll('.w-1\\/4').length).toBe(2);
        expect(container.querySelector('.flex-1.min-w-\\[400px\\]')).toBeInTheDocument();
      }), { numRuns: 20 });
    });
    ...
    test('should maintain panel proportions with correct width constraints', () => {
      fc.assert(fc.property(fc.boolean(), () => {
        const { container } = renderWithProvider();
        expect(container.querySelectorAll('.min-w-\\[280px\\]').length).toBe(2);
        expect(container.querySelectorAll('.max-w-\\[400px\\]').length).toBe(2);
      }), { numRuns: 20 });
    });
  ...
  describe('5.2 Real-time Workflow State Visualization', () => {
    ...
    test('should render all required left panel components', () => {
      fc.assert(fc.property(alertSeverityArb, () => {
        renderWithProvider();
  ...
  describe('5.3 Agent Activity and Findings Display', () => {
    test('should always render all three specialist agent cards', () => {
      fc.assert(fc.property(agentStatusArb, () => {
        renderWithProvider();
    ...
    test('should render specialist agents section in center panel', () => {
      fc.assert(fc.property(agentTypeArb, () => {
        renderWithProvider();
    ...
    test('should render blast radius section in center panel', () => {
      fc.assert(fc.property(riskLevelArb, () => {
        renderWithProvider();
  ...
  describe('Layout Consistency Across Data Variations', () => {
    test('should maintain layout with various confidence levels', () => {
      fc.assert(fc.property(confidenceLevelArb, (confidence) => {
    ...
    test('should maintain layout with various risk levels', () => {
      fc.assert(fc.property(riskLevelArb, (riskLevel) => {
  ...
  describe('Panel Border and Styling Consistency', () => {
    test('should have correct border styling on panels', () => {
      fc.assert(fc.property(fc.boolean(), () => {
        const { container } = renderWithProvider();
        expect(container.querySelector('.border-r.border-border')).toBeInTheDocument();
        expect(container.querySelector('.border-l.border-border')).toBeInTheDocument();
      }), { numRuns: 20 });
    });
  ...
  describe('Overflow and Scrolling Behavior', () => {
    test('should have overflow-hidden on main container', () => {
      fc.assert(fc.property(fc.boolean(), () => {
        const { container } = renderWithProvider();
        expect(container.querySelectorAll('.overflow-hidden').length).toBeGreaterThan(0);
      }), { numRuns: 20 });
    });
    ...
    test('should have scrollable content areas in panels', () => {
      fc.assert(fc.property(fc.boolean(), () => {
        const { container } = renderWithProvider();
        expect(container.querySelectorAll('.overflow-y-auto').length).toBeGreaterThan(0);
      }), { numRuns: 20 });
    });
  });
...
describe('Agent Card Display Properties', () => {
  ...
  test('should display agent cards with valid status indicators', () => {
    fc.assert(fc.property(agentStatusArb, (status) => {
  ...
  test('should handle all valid agent types', () => {
    fc.assert(fc.property(agentTypeArb, (agentType) => {
...
describe('Governance Request Display Properties', () => {
  ...
  test('should handle all valid health statuses for blast radius', () => {
    fc.assert(fc.property(healthStatusArb, (healthStatus) => {
...
describe('Workflow State Rendering Properties', () => {
  ...
  test('should render without errors for all workflow states', () => {
    fc.assert(fc.property(workflowStateArb, (state) => {
      const validStates = [
        WorkflowState.IDLE, WorkflowState.INCIDENT_INGESTED, WorkflowState.ANALYZING,
        WorkflowState.RCA_COMPLETE, WorkflowState.GOVERNANCE_PENDING, WorkflowState.ACTION_PROPOSED,
        WorkflowState.VERIFIED, WorkflowState.RESOLVED, WorkflowState.TERMINATED
